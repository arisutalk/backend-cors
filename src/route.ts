...
const route = new Hono()
    .all(
        ...
        describeRoute({
            ...
            description: "This endpoint allows you to make a request without cors blocking. It returns the response from the requested URL." +
        ...
        validator("header", inputSchema, i => {
            ...
            try {
                ...
                throw new TypeError("Invalid URL format, must be a valid URL-encoded string.");
            }
        }),
        async (c) => {
            //here
            ...
            const clientIp = c.req.header("CF-Connecting-IP") || getConnInfo(c).remote.address || undefined
            ...
            // Set CORS headers
            responseHeaders.set("Access-Control-Allow-Origin", // Since there might be authorization, we cannot use '*'
                c.req.header("Origin") || "*"
            )
            responseHeaders.set("Access-Control-Allow-Methods", "GET, HEAD, POST, OPTIONS, PUT, PATCH, DELETE")
            responseHeaders.set("Access-Control-Allow-Headers", // Allow all requested headers
